...
namespace trans {
  ...
  using std::string;
  AllocaTransformer::AllocaTransformer(Module& module) : TransformerHelper(module), buffer_initializer_(module, "typestr"){
    // Declare functions
  ...
  llvm::Constant* AllocaTransformer::LazySpecification(llvm::Type* allocated_type) {
    ...
    */
      return buffer_initializer_.LazyMemoBuff(type_str.c_str()); /*
  ...
  void AllocaTransformer::visitAllocaInst(llvm::AllocaInst &alloca) {
    ...
    if (allocated_type->isIntegerTy() or
	allocated_type->isArrayTy()) {
